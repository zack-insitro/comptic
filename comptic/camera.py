...
def gen_bayer_coupling_matrix(image_stack_rgb, pixel_offsets=(0, 0)):
    ...
    for color_index, frame in enumerate(image_stack_rgb):
        bayer_coupling_matrix[0, color_index] = np.mean(
            frame[pixel_offsets[0] :: 2, pixel_offsets[1] :: 2]
        )
        bayer_coupling_matrix[1, color_index] = np.mean(
            frame[pixel_offsets[0] :: 2, pixel_offsets[1] + 1 :: 2]
        )
        bayer_coupling_matrix[2, color_index] = np.mean(
            frame[pixel_offsets[0] + 1 :: 2, pixel_offsets[1] :: 2]
        )
        bayer_coupling_matrix[3, color_index] = np.mean(
            frame[pixel_offsets[0] + 1 :: 2, pixel_offsets[1] + 1 :: 2]
        )
...
def demosaic(
    frame, order="grbg", bayer_coupling_matrix=None, debug=False, white_balance=False
):
    ...
    """Demosaic a frame"""
    frame_out = yp.zeros(
        (int(yp.shape(frame)[0] / 2), int(yp.shape(frame)[1] / 2), 3),
    ...
    if bayer_coupling_matrix is not None:
        frame_vec = yp.zeros(
            (4, int(yp.shape(frame)[0] * yp.shape(frame)[1] / 4)),
            yp.getDatatype(frame),
            yp.getBackend(frame),
        )

        # Cast bayer coupling matrix
        bayer_coupling_matrix = yp.cast(
            bayer_coupling_matrix, yp.getDatatype(frame), yp.getBackend(frame)
        )
        ...
        for bayer_pattern_index in range(4):
            ...
            if debug:
                print(
                    "Channel %d mean is %g"
                    % (bayer_pattern_index, yp.scalar(yp.real(yp.sum(img_sub))))
                )
        ...
        for channel in range(3):
            ...
            frame_out[:, :, channel] = yp.reshape(
                values, ((yp.shape(frame_out)[0], yp.shape(frame_out)[1]))
            )
            if white_balance:
        ...
        frame_out = yp.zeros(
            (int(yp.shape(frame)[0] / 2), int(yp.shape(frame)[1] / 2), 3),
            dtype=yp.getDatatype(frame),
            backend=yp.getBackend(frame),
        )
        ...
        frame_out[:, :, 1] = (
            frame[g1_start[0] :: 2, g1_start[1] :: 2]
            + frame[g2_start[0] :: 2, g2_start[1] :: 2]
        ) / 2.0
...
def bayer_coupling_matrix(camera_label):
    ...
    if camera_label == "pco":
        ...
        # QImaging Optimos
        return [
            [0.21998870, 0.28237841, 0.26123319],
